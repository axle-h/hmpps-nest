import { ArgumentMetadata, Injectable, PipeTransform, Render, UsePipes } from '@nestjs/common';
...
export class ValidationPipe implements PipeTransform {
  ...
  async transform(model: any, { metatype }: ArgumentMetadata) {
    if (!metatype || [String, Boolean, Number, Array, Object].includes(metatype as any)) {
      return model;
    }
...
export function ValidateAndRender(template: string) {
  return function (target: any, propertyKey: string | symbol, descriptor: any): void {
